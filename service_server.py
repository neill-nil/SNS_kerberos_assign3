...
class ServiceHandler(socketserver.BaseRequestHandler):
    def handle(self):
        ...
        k_v = bytes.fromhex(config["services"][server.service_id]["k_v"])
        ...
        for sig_data in service_ticket.get("signatures", []):
            ...
            if auth_id not in config["tgs_nodes"]:
                continue
                
            pub_key = config["tgs_nodes"][auth_id]["public_key"]
            R = sig_data["R"]
            s = sig_data["s"]
        ...
        session_key_bytes = service_session_key.encode('utf-8')[:32]
...
def start_server(service_id):
    ...
    port = config["services"][service_id]["port"]
